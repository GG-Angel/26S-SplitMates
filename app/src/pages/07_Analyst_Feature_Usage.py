...
with col_stack:
    ...
    new Chart(document.getElementById('stackChart'), {{
        ...
        data: {{
            ...
            datasets: [
                {{ label: 'Create', data: {_cj.dumps(creates)}, backgroundColor: '#7f1d1d' }},
                {{ label: 'Update', data: {_cj.dumps(updates)}, backgroundColor: '#f87171' }},
                {{ label: 'Delete', data: {_cj.dumps(deletes)}, backgroundColor: '#7f1d1d' }}
            ]
        }},
        options: {{
...
with col_clicks:
    ...
    const hbar = new Chart(document.getElementById('hbarChart'), {{
        type: 'bar',
        data: {{ labels: fcData['All'].labels, datasets: [{{ data: fcData['All'].values, backgroundColor: '#7f1d1d' }}] }},
