...
def analyst_navs():
    st.sidebar.button(label="Nicole Parks", icon="👤", disabled=True)
    ...
    st.sidebar.page_link(
        "pages/08_Analyst_Sessions.py", label="User Sessions", icon="⏱️"
    )
    st.sidebar.page_link(
        "pages/09_Analyst_Inactive_Users.py", label="Inactive Users", icon="👤"
    )
...
def SideBarLinks(show_home=False):
    ...
    if st.session_state["authenticated"]:
        ...
        if role == "analyst":
            ...
            user_navs()
            st.sidebar.page_link(
                "pages/11_Submit_Report.py", label="Submit Report", icon="🚩"
            )
        st.sidebar.divider()
